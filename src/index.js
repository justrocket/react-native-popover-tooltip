...
class PopoverTooltip extends React.PureComponent<Props, State> {
    ...
    onInnerContainerLayout = (event: { nativeEvent: { layout: { height: number, width: number } } },) => {
        ...
        componentWrapper.measure((x, y, width, height, pageX, pageY) => {
            ...
            if (pageY - tooltipContainerHeight < 0) {
                tooltipContainerY_final = pageY + height;
                tooltipTriangleDown = false;
            }
            if (pageY + tooltipContainerHeight + 80 > window.height) {
                tooltipContainerY_final = pageY - tooltipContainerHeight;
            ...
            const tooltipContainerY = this.state.tooltipContainerScale.interpolate({
                inputRange: [0, 1],
                outputRange: [
                    tooltipContainerY_final + tooltipContainerHeight / 2,
